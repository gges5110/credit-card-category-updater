...
export const CategoryCard: React.FC<CategoryCardProps> = ({ categoryResult, calendarUrl }) => {
    ...
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-6">
        <div className="flex items-center mb-3">
          ...
          <h3 className="text-lg font-medium text-red-800 ml-3">{categoryResult.source}</h3>
        </div>
        <p className="text-red-700">Failed to load categories: {categoryResult.error}</p>
      </div>
    );
  }
    ...
    return (
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6">
        <div className="flex items-center mb-3">
          ...
          <h3 className="text-lg font-medium text-yellow-800 ml-3">{categoryResult.source}</h3>
        </div>
        <p className="text-yellow-700">No categories available yet. Check back later!</p>
      </div>
    );
  }

  return (
    <div className={`border rounded-lg p-6 transition-shadow duration-200 hover:shadow-md ${getSourceColor(categoryResult.source)}`}>
      ...
      <div className="flex items-center justify-between pt-4 border-t border-current border-opacity-20">
        <div className="text-sm opacity-75">
          ...
          <p className="mt-1">Updated: {new Date(categoryResult.timestamp).toLocaleDateString()}</p>
        </div>
